...
import benchmark.flops_benchmark

# from model import MobileNet2
from data_loader import get_loaders
from train_test import train, test, save_checkpoint, find_bounds_clr

#utilities import
# from utils import config #under work
from utils.logger import CsvLogger
from utils.clr import CyclicLR

#ssd imports
# from ssd_multibox.ssd import build_ssd
from model.layers.modules.multibox_loss import MultiBoxLoss

#model import
from model.model_main import create_model
...
parser.add_argument('-b', '--batch-size', default=2, type=int, metavar='N', help='mini-batch size (default: 64)') #64
...
parser.add_argument('--input-size', type=int, default=640, metavar='I',
...
def main():
    ...
    # model = torchvision.models.mobilenet_v2(pretrained=False, progress=True, num_classes=2, width_mult=1.0, inverted_residual_setting=None, round_nearest=8)
    ...
    if args.gpus is not None:
        model = torch.nn.DataParallel(ssd_net, args.gpus)
...
def train_network(start_epoch, epochs, scheduler, model, train_loader, val_loader, optimizer, criterion, device, dtype,
    ...
    for epoch in trange(start_epoch, epochs + 1):
        ...
        if not isinstance(scheduler, CyclicLR): #re-arraged to here to appease compiler
            scheduler.step()
